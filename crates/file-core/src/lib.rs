...
pub fn compress_to_zip<F: FnMut(Message)>(
    src: &str,
    dst: &str,
    mut call_back: Option<F>,
) -> anyhow::Result<()> {
    ...
    for it in &content_path {
        ...
        // unified path to linux style
        // The path would be A\B\C.file on Windows, however, A and A\B won't be recognized as directories on Linux
...
pub fn decompress_zip_to_dir<F: FnMut(Message)>(
    src: &str,
    dest: &str,
    mut call_back: Option<F>,
    ...
    for i in 0..archive.len() {
        ...
        println!("outpath {outpath:?}, file: {:?}", file.name());
        ...
        #[cfg(unix)]
        {
            ...
            if let Some(mode) = file.unix_mode() {
                match std::fs::set_permissions(&outpath, std::fs::Permissions::from_mode(mode)) {
                    ...
                    Err(e) => {
                        let err = format!(
                            "fail to give permission to \"{}\", reason:{e:?}",
                            relative_path.display()
                        );
                        fail_files.push(err);
                    }
