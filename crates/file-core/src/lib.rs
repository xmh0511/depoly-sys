	...
	for i in 0..archive.len(){
		...
		if (*file.name()).ends_with('/') {
            ...
            match std::fs::create_dir_all(&outpath){
				Ok(_)=>{}
				Err(e)=>{
					let err = format!("fail to create \"{}\", reason:{e:?}",relative_path.display());
					fail_files.push(err);
				}
			}
        } else {
            ...
            match std::io::copy(&mut file, &mut outfile){
                Ok(_) => {},
                Err(e) => {
					let err = format!("fail to place \"{}\", reason:{e:?}",relative_path.display());
					fail_files.push(err);
				},
            }
        }
		...
		#[cfg(unix)]
		{
			...
			if let Some(mode) = file.unix_mode() {
				match std::fs::set_permissions(&outpath, std::fs::Permissions::from_mode(mode)){
					Ok(_)=>{},
					Err(e)=>{
						let err = format!("fail to give permission to \"{}\", reason:{e:?}",relative_path.display());
						fail_files.push(err);
					}
				};
			}
